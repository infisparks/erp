...
import { 
    ...
    ArrowLeft, ArrowRight, Lock, RefreshCw, Upload,
...
export default function AdmissionForm({ user, onSuccess }: { user: any, onSuccess?: () => void }) {
    ...
    const [formData, setFormData] = useState<any>({
        ...
        payment_plan: "One Time", installment_letter: "",
        quota_selection: "", discipline: "", branch_preferences: "", how_did_you_know: "",
        ...
        try {
            ...
            if (student) {
                ...
                setFormData({
                    ...
                    quota_selection: student.quota_selection || "",
        ...
        const fetchFees = async () => {
            ...
            try {
                const feesRes = await calculateStudentFees(supabase, {
                    courseId: course_id, academicYearId: academic_year_id, scholarshipCategoryId: scholarship_category_id
                })
    ...
    const saveDraft = async () => {
        ...
        const studentData = {
            user_id: user.id,
            registration_no: formData.registration_no, merit_no: formData.merit_no,
            quota_selection: formData.quota_selection, discipline: formData.discipline,
            branch_preferences: formData.branch_preferences, how_did_you_know: formData.how_did_you_know,
            scholarship_category_id: formData.scholarship_category_id,
            admission_category: formData.scholarship_category_id, admission_type: formData.admission_type,
            fullname: formData.fullname, firstname: names[0] || 'Student',
            lastname: names.length > 1 ? names.slice(1).join(' ') : '.',
            father_name: formData.fatherName, mother_name: formData.motherName,
            father_occupation: formData.father_occupation, mother_occupation: formData.mother_occupation,
            father_annual_income: formData.father_annual_income,
            email: formData.email, dateofbirth: formData.dateOfBirth,
            admission_year_id: formData.academic_year_id, course_id: formData.course_id,
            gender: formData.gender, religion: formData.religion, caste: formData.caste,
            blood_group: formData.blood_group, aadhar_card_number: formData.aadharCardNumber,
            pan_no: formData.pan_no, student_mobile_no: formData.studentMobileNo,
            phone: formData.studentMobileNo, secondary_phone: formData.secondary_phone,
            ...
            photo_path: formData.photo_path, documents: formData.documents
        }
    ...
    return (
        <div className="min-h-screen bg-[#F5F7FB] pb-28" style={{ fontFamily: "'Poppins', sans-serif" }}>
            ...
            <div id="form-content" className="max-w-3xl mx-auto sm:px-3 px-0 pt-4">
                    ...
                    <div>
                        ...
                        <Section title="Academic Routing" icon={BookOpen}>
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <DD label="Scholarship" val={formData.scholarship_category_id} k="scholarship_category_id" opts={metadata?.scholarshipCategories} isObj icon={CreditCard} isLocked={isLocked} onChange={update} />
                            </div>
                            ...
                            {formData.academic_year_id && (
                                <>
                                    <DD label="Payment Plan" val={formData.payment_plan} k="payment_plan" opts={metadata?.staticOptions?.paymentPlans} icon={CreditCard} isLocked={isLocked} onChange={update} />
                                    {formData.payment_plan === "Installments" && (
                                        <TF label="Installment Undertaking" val={formData.installment_letter} k="installment_letter" isTA isLocked={isLocked} onChange={update} />
                                    )}
                                </>
                            )}

                            <div className="grid grid-cols-2 gap-3">
                                <DD label="Quota" val={formData.quota_selection} k="quota_selection" opts={metadata?.staticOptions?.quotas} icon={User} isLocked={isLocked} onChange={update} />
                                <DD label="Discipline" val={formData.discipline} k="discipline" opts={metadata?.staticOptions?.disciplines} icon={BookOpen} isLocked={isLocked} onChange={update} />
                            </div>
                            <TF label="Branch Preferences" val={formData.branch_preferences} k="branch_preferences" icon={Info} isLocked={isLocked} onChange={update} />
                        ...
                        <Section title="Personal & Family" icon={User}>
                            ...
                            <TF label="Full Legal Name" val={formData.fullname} k="fullname" icon={User} isLocked={isLocked} onChange={update} />
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <TF label="Annual Income" val={formData.father_annual_income} k="father_annual_income" type="number" icon={CreditCard} isLocked={isLocked} onChange={update} />
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-2">
                                <label className={`border ${formData.domicile_of_maharashtra === 'true' ? 'bg-[#2E75C7]/5 border-[#2E75C7]' : 'bg-gray-50 border-gray-100'} rounded-xl p-3 flex justify-between items-center cursor-pointer`}>
                                    <span className={`text-[10px] font-medium ${formData.domicile_of_maharashtra === 'true' ? 'text-[#1A3A6B]' : 'text-gray-400'}`}>MH Domicile</span>
                                    <input type="checkbox" className="accent-[#1A3A6B]" checked={formData.domicile_of_maharashtra === 'true'} onChange={e => update('domicile_of_maharashtra', e.target.checked ? 'true' : 'false')} />
                                </label>
                                <label className={`border ${formData.phd_handicap === 'true' ? 'bg-[#2E75C7]/5 border-[#2E75C7]' : 'bg-gray-50 border-gray-100'} rounded-xl p-3 flex justify-between items-center cursor-pointer`}>
                                    <span className={`text-[10px] font-medium ${formData.phd_handicap === 'true' ? 'text-[#1A3A6B]' : 'text-gray-400'}`}>Physically Handicapped</span>
                                    <input type="checkbox" className="accent-[#1A3A6B]" checked={formData.phd_handicap === 'true'} onChange={e => update('phd_handicap', e.target.checked ? 'true' : 'false')} />
                                </label>
                            </div>
                        </Section>
                    </div>
                ...
                {/* STEP 1: Address */}
                {currentStep === 1 && (
                    <Section title="Contact & Address" icon={Home}>
                        <div className="grid grid-cols-2 gap-3">
                            <TF label="Primary Mobile" val={formData.studentMobileNo} k="studentMobileNo" type="tel" icon={Info} isLocked={isLocked} onChange={update} />
                        ...
                        <TF label="Permanent Address" val={formData.address} k="address" isTA isLocked={isLocked} onChange={update} />
                        <div className="grid grid-cols-2 gap-3">
                        ...
                        <TF label="District" val={formData.district} k="district" icon={Info} isLocked={isLocked} onChange={update} />
                    </Section>
                )}
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-gray-100 px-3 py-3 shadow-lg z-40">
                <div className="max-w-3xl mx-auto flex gap-2.5">
                    ...
                    <button
                        ...
                        disabled={isSaving}
                        className="flex-1 h-12 rounded-xl bg-gradient-to-r from-[#1A3A6B] to-[#2E75C7] text-white flex items-center justify-center gap-2.5 font-medium text-sm shadow-lg shadow-[#2E75C7]/20 hover:shadow-[#2E75C7]/40 transition-all active:scale-95 disabled:opacity-60"
                    >
                        {isSaving ? (
