...
export default function AdmissionForm({ user }: AdmissionFormProps) {
  ...
  useEffect(() => {
    ...
    const fetchFormConfig = async () => {
      try {
        ...
        const [customData, docData, courseData, categoryData] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const requiredFields: (keyof FormData)[] = [
        ...
        "admission_year", "course_id", "admission_category"
    ];
      ...
      <Card className="border-0 shadow-lg rounded-lg overflow-hidden bg-white">
        ...
        <CardContent className="p-4 md:p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* --- 1. Academic Details (NEW SECTION) --- */}
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  {/* 🔧 FIX: Removed inputClassName, added p-inputtext-sm to className */}
                  <Dropdown
                    name="admission_year"
